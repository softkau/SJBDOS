...
namespace {
  ...
  void SwitchEhci2Xhci(const pci::Device& xhc_dev) {
	  // search for ehc device
	  bool intel_ehc_exist = false;
	  for (int i = 0; i < pci::g::num_devices; i++) {
	  	if (pci::g::devices[i].class_code.match(0x0C, 0x03, 0x20) &&
	  		pci::g::devices[i].vendor_id == 0x8086) {
	  ...
	  // XCHI speed boost
	  uint32_t superspeed_ports = pci::ReadConfigSpace(xhc_dev, 0xDC); // USB3PRM reg: effective bits for superspeed ports
	  pci::WriteConfigSpace(xhc_dev, 0xD8, superspeed_ports); // USB3_PSSEN reg: superspeed-state bitset

	  // EHCI -> XHCI
	  uint32_t ehci2xhci_ports = pci::ReadConfigSpace(xhc_dev, 0xD4); // XUSB2PRM reg: effective bits for xhci ports
	  pci::WriteConfigSpace(xhc_dev, 0xD0, ehci2xhci_ports); // XUSB2PR reg: xchi enabled-state bitset
...
namespace usb::xhci {
  ...
  pci::Device* FindMainXHC() {
  	pci::Device* xhc_dev = nullptr;
  	for (int i = 0; i < pci::g::num_devices; i++) {
  		auto& dev = pci::g::devices[i];
  		auto& cc= dev.class_code;
  		if (cc.match(0x0C, 0x03, 0x30)) {
  			xhc_dev = &dev;
  			if (xhc_dev->vendor_id == 0x8086) break; // probably the main XHC-Interface
  		}
  	}
  	return xhc_dev;
  }

  inline uint8_t GetCurrentLocalAPICID() { return *reinterpret_cast<uint32_t*>(0xFEE00020) >> 24; }

  void Initialize() {
    // Intel 製を優先して xHC を探す
    pci::Device* xhc_dev = FindMainXHC();
    ...
    const auto bootstrap_proc_apic_id = GetCurrentLocalAPICID();
      pci::ConfigureMSIFixedDestination(
	  	*xhc_dev,
	  	bootstrap_proc_apic_id,
	  	pci::MSITriggerMode::Level,
	  	pci::MSIDeliveryMode::Fixed,
	  	InterruptVector::XHCI, 0
	  );

    auto xhc_bar = pci::ReadDeviceBaseAddress(*xhc_dev, 0);
	  if (!xhc_bar.has_value) {
	  	Log(kError, "while reading: %s\n", xhc_bar.error.GetName());
	  	exit(1);
	  }
    uint64_t xhc_mmio_base = *xhc_bar >> 4 << 4;
	  Log(kDebug, "xHC mmio_base: %08lX\n", xhc_mmio_base);

	  usb::xhci::kController = new Controller{xhc_mmio_base};
	  Controller& xhc = *usb::xhci::kController;

    if (0x8086 == xhc_dev->vendor_id)
	  	SwitchEhci2Xhci(*xhc_dev);

	  if (auto err = xhc.Initialize()) {
	  	Log(kError, "xhc.init[%s]\n", err.GetName());
      exit(1);
    }

	  Log(kInfo, "xHC starting!\n");
	  xhc.Run(); // run xHC device
	  	...
	  	if (port.IsConnected()) {
	  		if (auto err = ConfigurePort(xhc, port)) {
	  			Log(kError, "failed to configure port: %s at %s:%d\n",
	  				err.GetName(), err.GetFile(), err.GetLine());
	  			continue;
	  		}
	  	}
	  }
  ...
  void ProcessEvents() {
    while (kController->PrimaryEventRing()->HasFront()) {
      if (auto err = ProcessEvent(*kController)) {
